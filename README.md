...
Checks F and G are skipped when `DOMAIN` is empty or `SKIP_*` flags are set. Check J is skipped when `--external-db` is not provided.
...
The test suite runs unit tests (normalization helpers, kernel version parsing, mock-kubectl pass/fail cases, python3-absence check, external-DB check) and integration tests against a real cluster. All 47 unit tests must pass; integration tests require `--kubeconfig=` and optionally `--external-db=`.

```bash
# Unit tests only
bash kcs_k8s_check_test.sh

# Full integration including external PostgreSQL
bash kcs_k8s_check_test.sh \
